...
import { renderAllUsers } from '../cabinet/all-users.ts';
import { runApplication } from '../cabinet/volonteer-application.ts';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if(!allProd || !allUsers || !payments || !applications) return;
  ...
  await renderAllUsers(allUsers, allProd ,payments, applications);
  await runApplication(applications, allUsers);
  burgerMenu();
  redirect();
  initTab();
  initDropdownMenu();
